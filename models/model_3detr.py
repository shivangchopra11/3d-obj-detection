...
class Model3DETR(nn.Module):
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features, final_feats=False):
        """
        Parameters:
    ...
    def forward(self, inputs, encoder_only=False, final_feats=False):
        ...
        if final_feats:
            box_predictions = self.get_box_predictions(
                query_xyz, point_cloud_dims, box_features, final_feats=True
            )
            # print('Box Preds', box_predictions.shape)
            return query_xyz, box_predictions, point_clouds
