...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        for cat in self.categories:
            vc = np.load('./features/' + cat + '/center_dict.pickle', allow_pickle=True)
            vc = vc[:,  :, np.newaxis]
            # print('VC', vc.shape)
            vc = torch.from_numpy(vc).type(torch.FloatTensor).to('cuda:0')
            self.conv.append(Conv1o1Layer(vc))
        # exit()
        # self.output_heads = []
        self.center_output = nn.Conv1d(100, 3, 1, bias=True)
    ...
    def get_feature_vectors(self, query_xyz, point_cloud_dims, box_features, final_feats=False):
        """
        Parameters:
        ...
        # if final_feats:
        #     print(box_features.transpose(1, 2).shape)
        #     # exit()
        #     return query_xyz, box_features.transpose(1, 2)
        ...
        if final_feats:
            ...
            return query_xyz, point_cloud_dims, cls_logits, center_offset, size_normalized, angle_logits, angle_residual_normalized, angle_residual

    def get_box_predictions(self, query_xyz, point_cloud_dims, cls_logits, center_offset, size_normalized, angle_logits, angle_residual_normalized, angle_residual):
        """
        Parameters:
            query_xyz: batch x nqueries x 3 tensor of query XYZ coords
            point_cloud_dims: List of [min, max] dims of point cloud
    ...
    def forward(self, inputs, encoder_only=False, final_feats=False):
        ...
        query_xyz, point_cloud_dims, cls_logits, center_offset, size_normalized, angle_logits, angle_residual_normalized, angle_residual = self.get_feature_vectors(
            query_xyz, point_cloud_dims, box_features, final_feats
        )
        if final_feats:
            return query_xyz, cls_logits[-1,:,:,:]

        print('Feature Vector', cls_logits.shape)
        ...
        act_list = []
        for idx, con in enumerate(self.conv):
          # print('Enc feats in', enc_features.shape)
          activations = con(cls_logits[-1])
          # print('Activations', activations.shape)
          act_list.append(activations)
        all_acts = torch.cat(act_list, dim=1)

        print('Conv1o1 Output', all_acts.shape)

        all_acts_head = self.output_act(self.center_output(all_acts))
        print('All Acts Output', all_acts_head.shape)

        box_predictions = self.get_box_predictions(query_xyz, point_cloud_dims, all_acts_head, center_offset, size_normalized, angle_logits, angle_residual_normalized, angle_residual)

        # box_predictions = self.get_feature_vectors(
        #     query_xyz, point_cloud_dims, box_features, final_feats
        # )
