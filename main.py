...
def make_args_parser():
    ...
    parser.add_argument("--batchsize_per_gpu", default=20, type=int)
...
def do_train(
    ...
    print('TrainableParams', sum([p.numel() for p in model.parameters() if p.requires_grad]))
...
def main(local_rank, args):
    ...
    print(model.mlp_heads.sem_cls_head.layers[0].weight[0][0])

    sd["model"].pop('mlp_heads.sem_cls_head.layers.8.weight')
    sd["model"].pop('mlp_heads.sem_cls_head.layers.8.bias')
    sd["model"].pop('mlp_heads.center_head.layers.8.weight')
    sd["model"].pop('mlp_heads.center_head.layers.8.bias')
    sd["model"].pop('mlp_heads.size_head.layers.8.weight')
    sd["model"].pop('mlp_heads.size_head.layers.8.bias')
    sd["model"].pop('mlp_heads.angle_cls_head.layers.8.weight')
    sd["model"].pop('mlp_heads.angle_cls_head.layers.8.bias')
    sd["model"].pop('mlp_heads.angle_residual_head.layers.8.weight')
    sd["model"].pop('mlp_heads.angle_residual_head.layers.8.bias')

    model.load_state_dict(sd["model"], strict=False)

    print(sd["model"]['mlp_heads.sem_cls_head.layers.0.weight'][0][0])
    print(model.mlp_heads.sem_cls_head.layers[0].weight[0][0])
    ...
    layers = [
        # model.mlp_heads.sem_cls_head.layers[8],
        model.mlp_heads.sem_cls_head.layers[9],
        # model.mlp_heads.center_head.layers[8],
        model.mlp_heads.center_head.layers[9],
        # model.mlp_heads.size_head.layers[8],
        model.mlp_heads.size_head.layers[9],
        # model.mlp_heads.angle_cls_head.layers[8],
        model.mlp_heads.angle_cls_head.layers[9],
        # model.mlp_heads.angle_residual_head.layers[8],
        model.mlp_heads.angle_residual_head.layers[9],
    ]
